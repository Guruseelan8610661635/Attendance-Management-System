...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleRoleSelect = (role: UserRole) => {
    ...
    setStep('login-form');
    // Pre-fill for convenience in demo
    if (role === UserRole.STAFF) {
      setEmail('staff1@gmail.com');
    } else if (role === UserRole.ADMIN) {
      setEmail('sarah.admin@attendx.edu');
    } else {
      setEmail('alex@attendx.edu');
    }
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      if (selectedRole) onLogin(selectedRole, email);
    }, 1200);
  };
